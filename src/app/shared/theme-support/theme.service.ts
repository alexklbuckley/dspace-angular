...
import { CollectionStylesDataService } from './collection-styles.service';
	...
	this.addCollectionCSSToHead('');
	    ...
	    // If snapshot is a collection then we are on the Collection Browse page
            ...
            if (snapshotWithData.data.dso.payload.type === 'collection') {
              if (this.doesCollectionUUIDMatchThemeConfig(snapshotWithData.data.dso.payload.id)) {
                if (hasValue(snapshotWithData.data.dso.payload.cssDisplay)) {
                  ...
                  return observableOf(snapshotWithData.data.dso.payload).pipe(
...
/*
		  ...
		  // check if either owning or mapped collections has specific theme rule
                  collections$.subscribe((collections: Collection[]) =>
...
*/
  ...
  /**
   * Check if a collection UUID has a theme mapped to it in the config.*.yml
   */
  doesCollectionUUIDMatchThemeConfig(uuid: string) {
    let match = environment.themes.find((themeConfig: any) =>
      hasValue(themeConfig.uuid) &&
      themeConfig.uuid == uuid
    );
    let matchingTheme = this.themes.find((theme: Theme) => match);
    if (match) {
      this.collectionStylesDataService.setStyledCollectionName(uuid);
      return uuid;
    } else {
      return null;
    }
